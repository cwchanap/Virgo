...
Revalidation on 2026-08-22 after HPA-581 merged confirmed that latest `main` still has the same mapper, file-manager, downloader, and gameplay seams above. HPA-581's notation/performance changes do not alter this design or expand HPA-85's production scope.
...
The latest test suite also contains format-agnostic GraphQL/catalog fixtures that use `bgm.ogg` as representative server data. Keep their purpose narrow while making current published-server examples agree with the new contract:

- update representative R2 BGM keys in `ApolloSimfileClientTests.swift` and `GraphQLQuerySchemaTests.swift` to `bgm.m4a`;
- update the server DTO `fileKeys` in `ServerSongCatalogRefreshTests.swift` to `bgm.m4a` when that fixture represents the refreshed catalog;
- do not mass-rewrite local `Song.bgmFilePath` fixtures that intentionally model stale development data or merely test extension-agnostic path/status behavior.

The audit is semantic, not a repository-wide extension replacement.
...
- `VirgoTests/ServerSongFileManagerTests.swift`
- `VirgoTests/ServerSongDownloaderTests.swift`
- `VirgoTests/ApolloSimfileClientTests.swift` (current server-key fixtures only)
- `VirgoTests/GraphQLQuerySchemaTests.swift` (current server-key fixtures only)
- `VirgoTests/ServerSongCatalogRefreshTests.swift` (current server DTO key only; preserve intentional stale local path fixtures)
