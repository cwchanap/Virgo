...
extension MetronomeEngine {
    /// Synchronous start for testing - waits for state update to complete
    func startSync(bpm: Double, timeSignature: TimeSignature) async {
        ...
        // Wait for Combine publisher to deliver state update
    ...
    /// Synchronous stop for testing - waits for state update to complete
    func stopSync() async {
        timingEngine.stop()
        audioDriver.stop()
    ...
    /// Synchronous toggle for testing - waits for state update to complete
    func toggleSync(bpm: Double, timeSignature: TimeSignature) async {
        if isEnabled {
            await stopSync()
        } else {
            await startSync(bpm: bpm, timeSignature: timeSignature)
        }
    }
}
#endif
