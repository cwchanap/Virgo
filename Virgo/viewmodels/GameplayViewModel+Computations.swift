...
extension GameplayViewModel {
    func makeRhythmRuntime(resolvedRhythm: ResolvedChartRhythm) -> GameplayRhythmRuntime {
        ...
        do {
            ...
            return GameplayRhythmRuntime(
                ...
                noteTargets: makeNoteTargets(resolvedRhythm: resolvedRhythm, timeline: timeline),
    ...
    /// Timeline note targets ordered by one-X target seconds, then event ID for
    /// deterministic ordering of simultaneous events.
    private func makeNoteTargets(
        resolvedRhythm: ResolvedChartRhythm,
        timeline: RhythmTimeline
    ) -> [RhythmNoteTarget] {
        guard let track else { return [] }
        return resolvedRhythm.orderedEvents.compactMap { event -> RhythmNoteTarget? in
