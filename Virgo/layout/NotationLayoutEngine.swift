...
/// Legacy `NotationLayout` primitive builders — restored in HPA-166 Task 7
/// review as the *disconnected* app model: production preparation ends in the
/// package `EngravedNotation` and never calls this engine; only the restored
/// pure tests drive it (via `legacyPreparedNotation` in the test target).
/// Task 8 deletes this model outright.
///
/// The measured formatter (HPA-164) owned every X: measure bounds and rows
